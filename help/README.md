...
- `SERVE_AUTO_OPEN`, `SERVE_MINAPK_WEBVIEW`, and `SERVE_RANDOM_URL` set the
  ...
  `on`, plus a digit string for `SERVE_MINAPK_WEBVIEW`); every flag defaults
  to off/not-passed. A CLI flag always overrides its environment default,
  including turning off something the environment turned on.
